...
def _count(map_: dict[tuple[int, int], int], initial_pos: tuple[int, int]) -> int:
    ...
    to_do: set[tuple[tuple[int, int], ...]] = {(initial_pos,)}
    while to_do:
        *prevs, pos = to_do.pop()
        h = map_[pos]
        ...
        for next_pos in ((r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)):
            if next_pos not in done:
                if map_.get(next_pos, 0) == h + 1:
                    to_do.add((*prevs, pos, next_pos))

        done.add((*prevs, pos))

    return sum(1 for path in done if map_[path[-1]] == 9)
...
assert solve(TEST_DATA) == 81

print(solve(INPUT_DATA))  # 1252
